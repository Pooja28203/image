...
if (isset($_POST['submit']))
{
    ...
    if (!empty($_FILES['file']['name']) && in_array($_FILES['file']['type'], $fileMimes))
    {
            ...
            while (($getData = fgetcsv($csvFile, 10000, ",")) !== FALSE)
            {
                ...
                $leg_opening_in = $getData[13];
                $leg_opening_cm = $getData[14];
                

                $query = "SELECT id FROM womens_sh_dungarees WHERE sku = '" . $getData[0] . "'";
 
                $check = mysqli_query($connection, $query);
 
                if ($check->num_rows > 0)
                {
                    $result = mysqli_query($connection, "UPDATE womens_sh_dungarees SET waist_in = '" . $waist_in . "', waist_cm = '" . $waist_cm . "', hip_in = '" . $hip_in . "', hip_cm = '" . $hip_cm . "', rise_in = '" . $rise_in . "', rise_cm = '" . $rise_cm . "', bib_height_in = '" . $bib_height_in . "', bib_height_cm = '" . $bib_height_cm . "', inseam_in = '" . $inseam_in . "', inseam_cm = '" . $inseam_cm . "', outseam_in = '" . $outseam_in . "', outseam_cm = '" . $outseam_cm . "', leg_opening_in = '" . $leg_opening_in . "', leg_opening_cm = '" . $leg_opening_cm . "'");
                }
                else
                {
                    $result = mysqli_query($connection, "INSERT INTO womens_sh_dungarees (sku, waist_in, waist_cm, hip_in, hip_cm, rise_in, rise_cm, bib_height_in, bib_height_cm, inseam_in, inseam_cm, leg_opening_in, leg_opening_cm, outseam_in, outseam_cm) VALUES ('" . $sku . "','" . $waist_in . "','" . $waist_cm . "','" . $hip_in . "','" . $hip_cm . "','" . $rise_in . "','" . $rise_cm . "','" . $bib_height_in . "','" . $bib_height_cm . "','" . $inseam_in . "','" . $inseam_cm . "','" . $outseam_in . "','" . $outseam_cm . "','" . $leg_opening_in . "','" . $leg_opening_cm . "')");
                }
                ...
                $file_name='templates/womens/dungarees/shorts/jpg/template.jpg';
                ...
                imagejpeg($img_source,"outputs/womens/dungarees/shorts/jpg/".$sku."_M.jpg",100);
                imagedestroy($img_source);

                header("Location: womens-dungarees-sh-import.php");
