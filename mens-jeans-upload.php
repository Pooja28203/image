...
if (isset($_POST['submit']))
{
    ...
    if (!empty($_FILES['file']['name']) && in_array($_FILES['file']['type'], $fileMimes))
    {
            ...
            while (($getData = fgetcsv($csvFile, 10000, ",")) !== FALSE)
            {
                ...
                $query = "SELECT id FROM mens_jeans WHERE sku = '" . $getData[0] . "'";
 
                $check = mysqli_query($connection, $query);
 
                if ($check->num_rows > 0)
                {
                    $result = mysqli_query($connection, "UPDATE mens_jeans SET waist_in = '" . $waist_in . "', waist_cm = '" . $waist_cm . "', hip_in = '" . $hip_in . "', hip_cm = '" . $hip_cm . "', rise_in = '" . $rise_in . "', rise_cm = '" . $rise_cm . "', inseam_in = '" . $inseam_in . "', inseam_cm = '" . $inseam_cm . "', leg_opening_in = '" . $leg_opening_in . "', leg_opening_cm = '" . $leg_opening_cm . "', outseam_in = '" . $outseam_in . "', outseam_cm = '" . $outseam_cm . "'");
                }
                else
                {
                    $result = mysqli_query($connection, "INSERT INTO mens_jeans (sku, waist_in, waist_cm, hip_in, hip_cm, rise_in, rise_cm, inseam_in, inseam_cm, leg_opening_in, leg_opening_cm, outseam_in, outseam_cm) VALUES ('" . $sku . "','" . $waist_in . "','" . $waist_cm . "','" . $hip_in . "','" . $hip_cm . "','" . $rise_in . "','" . $rise_cm . "','" . $inseam_in . "','" . $inseam_cm . "','" . $leg_opening_in . "','" . $leg_opening_cm . "','" . $outseam_in . "','" . $outseam_cm . "')");
                }
                ...
                imagejpeg($img_source,"outputs/mens/jeans/jpg/".$sku."_M.jpg",100);
                imagedestroy($img_source);

                header("Location: mens-jeans-import.php");
